  document.addEventListener("DOMContentLoaded", () => {
    ...
    const exportSkippedPDFBtn = document.getElementById('exportSkippedPDF'); // زرار الـ PDF في الـ Modal
    ...
    const defaultDates = [
			...
			{ reason: "Eid Alfetr", date: "2024-04-13" },
			{ reason: "Foundation Day", date: "2026-02-22" },
			{ reason: "Eid al-Fitr", date: "2026-03-21" },
			{ reason: "Eid al-Fitr", date: "2026-03-22" },
			{ reason: "Eid al-Fitr", date: "2026-03-23" },
			{ reason: "Eid al-Fitr", date: "2026-03-24" },
			{ reason: "Eid al-Adha", date: "2026-05-26" },
			{ reason: "Eid al-Adha", date: "2026-05-27" },
			{ reason: "Eid al-Adha", date: "2026-05-28" },
			{ reason: "Eid al-Adha", date: "2026-05-30" },
			{ reason: "Saudi National Day", date: "2026-09-23" },
		];
    ...
    const getWeekNumber = (dateString) => {
      ...
      const offset = (firstDayOfMonth.getDay() + 1) % 7; 
    ...
    const calculateDateDiff = () => {
      if (!startDateDiff.value || !endDateDiff.value) {
        showToast("⚠ Please fill start and end date!", "danger");
        return;
      }
    ...
    const addHoliday = () => {
      ...
      if (!reason || !date) {
        showToast("⚠ Please fill both date and reason!", "danger");
        return;
      }
    ...
    /* ==================[ PDF Export Function ]================== */
    function exportSkippedDatesToPDF() {
      ...
      fetch(logoUrl)
        ...
        .then(logoData => {
          doc.addImage(logoData, "PNG", 150, 5, 40, 15);
          doc.setFontSize(16);
          doc.setFont("helvetica", "bold");
          doc.text("Skipped Dates Report", 14, 15);
          const startDate = document.getElementById('start-date')?.value || "N/A";
          const endDate = document.getElementById('end-date')?.value || "N/A";
          const skippedDays = skippedDatesGlobal.length;
          const workingDays = parseInt(document.getElementById('number-of-days')?.value || 0) || 0;
          doc.setFontSize(10);
          doc.setFont("helvetica", "normal");
          doc.text(`Start Date: ${startDate}`, 14, 25);
          doc.text(`End Date: ${endDate}`, 14, 30);
          doc.text(`Working Days: ${workingDays}`, 14, 35);
          doc.text(`Skipped Days: ${skippedDays}`, 14, 40);
          const tableRows = [];
          document.querySelectorAll("#skippedTableBody tr").forEach(row => {
            const cols = row.querySelectorAll("td");
            if (cols.length === 2) {
              tableRows.push([cols[0].innerText, cols[1].innerText]);
            }
          });
          doc.autoTable({ head: [["Date", "Reason"]], body: tableRows, startY: 50 });
          doc.setFontSize(9);
          doc.text(`Generated on: ${new Date().toLocaleString()}`, 14, doc.internal.pageSize.height - 10);
          doc.save(`Skipped_Dates_${new Date().toISOString().split('T')[0]}.pdf`);
        });
    }
    ...
    importInput?.addEventListener("change", (e) => {
      ...
      reader.onload = (event) => {
        ...
        let importedData = XLSX.utils.sheet_to_json(firstSheet);
    ...
    exportSkippedPDFBtn?.addEventListener("click", exportSkippedDatesToPDF);
